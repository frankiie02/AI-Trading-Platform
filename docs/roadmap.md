...
- **Broker interface + `PaperBroker` adapter** (`core/broker/`): a stable, broker-neutral `BrokerInterface` (`ABC`) contract - account/position/order/fill operations, typed models, a controlled error hierarchy - implemented by `PaperBroker`, which adapts the existing `PaperTradingService`/`PortfolioService` without duplicating any order-lifecycle, fill, or analytics logic. Establishes the abstraction `IBKRBroker` (below) now implements identically. Two small additions to `PaperTradingService` (`get_order()`, `get_open_orders()`); no other existing service, page, or runtime changed. Not wired into any Streamlit page or runtime mode - that is deliberately deferred.
- **Read-only `IBKRBroker` adapter** (`core/broker/ibkr_broker.py`, `core/broker/ibkr_client.py`): the first real (non-paper) `BrokerInterface` implementation, connecting to Interactive Brokers via the optional `ib_insync` dependency for **account/position/order/fill inspection only**. `submit_order()`/`cancel_order()` always raise `BrokerUnsupportedOperationError`; no order-transmission code exists anywhere in either file (statically proven by `tests/unit/test_ibkr_broker.py`, 92 tests against a fake client). `IBKRConnectionConfig` fails closed on `read_only=False` or `environment=LIVE`. Importing `core.broker` never requires `ib_insync` to be installed - only constructing a real (non-fake) client does, and a missing package surfaces as a controlled `BrokerConnectionError`. Reachable via `core.broker.factory.create_ibkr_broker()`; not wired into any Streamlit page, runtime mode, or `ExecutionRouter`. An opt-in integration test (`tests/integration/test_ibkr_broker_integration.py`, skipped unless `RUN_IBKR_INTEGRATION_TESTS=1`) exists for manual verification against a real IB Gateway/TWS paper account.
...
- IBKR order submission/cancellation (would require lifting `IBKRBroker`'s permanent read-only boundary - a deliberate, separate future decision, not an oversight)
- IBKR live trading integration
- Alpaca / OANDA broker adapters, behind the same `BrokerInterface`
- Wiring `BrokerInterface`/`PaperBroker`/`IBKRBroker` into a Streamlit page or the paper runtime
...
- `IBKRBroker` fetches no live market data: `BrokerPosition.current_price`/`market_value`/`unrealised_pnl` are always `None` when returned by `IBKRBroker`; `reserved_cash` is always reported as `0.0` (no IBKR equivalent surfaced in this milestone)
