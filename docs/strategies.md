...
`BacktestService` (`core/services/backtest_service.py`) reuses the same `TradingPipeline.evaluate()` call, once per historical bar (on an anti-lookahead slice of data through that bar), for both single-strategy and voting backtests — see "Backtest Service" in `docs/architecture.md`. No strategy, voting, regime, or risk logic is duplicated for backtesting.